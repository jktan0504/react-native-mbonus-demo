...
import UserPendingRebateList from '../../parts/UserPendingRebateList/UserPendingRebateList';
import { changeSettingsLocalization
} from '../../../../../../controllers/actions';
import { getAllPendingRebates
...
class UserPendingRebateFilterResultComponent extends Component {
    ...
    getAllPendingRebateList = (start_item, fetch_length,) => {
        console.log('running getAllPendingRebateList');
        const submitParams = {
            start_item,
            fetch_length,
        ...
        getAllPendingRebates(submitParams)
            .then((allTransactions) => {
                console.log(`result filter ${allTransactions.recordsFiltered}`);
                if (allTransactions.recordsFiltered <= 0) {
                    this.setState({
                        emptyList: true,
                        isLoading: false,
                    });
                    ...
                    if (allTransactions.data > 0) {
                        this.setState({
                            pendingRebateList: this.state.page_loading ===  1 ? allTransactions.data : [...this.state.pendingRebateList, ...allTransactions.data],
                            isRefreshing: false,
                            isLoading: false,
                            emptyList: false,
                        });
                        ...
                        if (this.state.start_item < allTransactions.recordsTotal) {
                            this.handlePaginationLoadMoreTransactionList();
                        }
                    }
    ...
    renderPendingList() {
        console.log(`result of pending ${this.state.emptyList}`);
        if (this.state.emptyList === true ) {
            return (
                ...
                >
                    {strings('pending_rebate.pending_rebate_empty')}
                </Text>
            );
        }
        else {
            return (
                <UserPendingRebateList
                    navigation={this.props.navigation}
                    transactionList={this.state.pendingRebateList}
...
})(UserPendingRebateFilterResultComponent);
